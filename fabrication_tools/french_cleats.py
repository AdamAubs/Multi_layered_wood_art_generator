...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="DPI used to convert mm dimensions into PNG pixels (defaults to package settings or 300).",
    )
    parser.add_argument(
        "--dpi-x",
        type=float,
        default=None,
        help="Horizontal DPI used to convert cleat dimensions into PNG pixels.",
    )
    parser.add_argument(
        "--dpi-y",
        type=float,
        default=None,
        help="Vertical DPI used to convert cleat dimensions into PNG pixels.",
    )
    parser.add_argument(
        "--top-inset-mm",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Frame margin passed through to DXF regeneration (defaults to package settings or 5).",
    )
    parser.add_argument(
        "--frame-margin-x-mm",
        type=float,
        default=None,
        help="Horizontal frame margin passed through to DXF regeneration.",
    )
    parser.add_argument(
        "--frame-margin-y-mm",
        type=float,
        default=None,
        help="Vertical frame margin passed through to DXF regeneration.",
    )
    parser.add_argument(
        "--setting-hole-diameter-mm",
        type=float,
        default=None,
        help="Setting-hole diameter passed through to DXF regeneration (defaults to package settings or 2.5).",
    )
    parser.add_argument(
...
def build_keyhole_mask(shape, args):
    ...
    top = mm_to_px(args.top_inset_mm, args.dpi_y)
    entry_radius_x = mm_to_px(args.entry_diameter_mm / 2.0, args.dpi_x)
    entry_radius_y = mm_to_px(args.entry_diameter_mm / 2.0, args.dpi_y)
    throat_width = max(1, mm_to_px(args.throat_width_mm, args.dpi_x))
    throat_height = max(1, mm_to_px(args.throat_height_mm, args.dpi_y))

    center_x = width // 2
    circle_center_y = top + throat_height + entry_radius_y
    ...
    if circle_center_y + entry_radius_y >= height:
...
def build_cavity_mask(shape, args):
    ...
    entry_radius_y = mm_to_px(args.entry_diameter_mm / 2.0, args.dpi_y)
    throat_height = max(1, mm_to_px(args.throat_height_mm, args.dpi_y))
    cavity_radius_x = max(1, mm_to_px(args.cavity_width_mm / 2.0, args.dpi_x))
    cavity_radius_y = max(1, mm_to_px(args.cavity_width_mm / 2.0, args.dpi_y))
    cavity_extension = max(1, mm_to_px(args.cavity_extension_mm, args.dpi_y))

    center_x = width // 2
    circle_center_y = top + throat_height + entry_radius_y
    cavity_top = max(0, top - cavity_extension)
    cavity_bottom = min(height - 1, circle_center_y + entry_radius_y + cavity_extension)
    ...
    cv2.rectangle(
        mask,
        (max(0, center_x - cavity_radius_x), cavity_top),
        (min(width - 1, center_x + cavity_radius_x), cavity_bottom),
        255,
        thickness=-1,
    )
    cv2.ellipse(
        mask,
        (center_x, cavity_top),
        (cavity_radius_x, cavity_radius_y),
        0,
        0,
        360,
        255,
...
def regenerate_dxf_with_frame(png_path, args):
    ...
    script_path = os.path.join(project_root, "png-to-dxf.py")
    command = [
        sys.executable,
        script_path,
        "--png",
        ...
        "--frame-margin-x-mm",
        f"{args.frame_margin_x_mm}",
        "--frame-margin-y-mm",
        f"{args.frame_margin_y_mm}",
        "--setting-hole-diameter-mm",
...
def refresh_layout_cut_generator(final_dir, stock_size_in, gap_mm):
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    layout_script = os.path.join(project_root, "layout_cut_generator.py")
    layout_cmd = [
        sys.executable,
        layout_script,
        "--dir",
...
def main():
    ...
    package_settings = load_dxf_settings(directory)
